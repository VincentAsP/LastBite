...
// ─── Constants ───
const CATEGORIES = ['Homemade', 'Bakery', 'Today\'s Leftover', 'Snack', 'Drinks', 'Other'];
const SALES_DATA = [
  { day: 'M', value: 7 },
  { day: 'T', value: 11 },
  { day: 'W', value: 6 },
            ...
            {cells.map((d, i) => {
              ...
              return (
                <Pressable
                  ...
                  disabled={past}
                >
                  <Text style={[cal.cellText, sel && cal.cellTextSel, past && cal.cellTextPast]}>
                    {d}
                  </Text>
                </Pressable>
              );
...
const cal = StyleSheet.create({
  ...
  weekLabel: {
    ...
    fontSize: 11, fontWeight: '600', color: '#9BA89B',
  },
...
export default function SellerHomepage() {
  ...
  const handleLaunch = () => {
    if (!foodName.trim() || !stockQty || !price || !photo) {
      Alert.alert('Incomplete', 'Please fill in food name, photo, stock quantity, and price.');
      return;
    }
    console.log('Launch food:', { foodName, description, category: activeCategory, stockQty, price, originalPrice, availableUntil: `${formatDate(selectedDate)} ${selectedTime}`, photo });
    Alert.alert('Listed Successfully 🚀', `${foodName} is now available for buyers!`, [{
      text: 'OK',
      onPress: () => {
        setFoodName(''); setDescription(''); setStockQty('');
        setPrice(''); setOriginalPrice('');
        setSelectedDate(new Date()); setSelectedTime('18:00'); setPhoto(null);
      },
    }]);
  };

  const totalRevenue = SOLD_HISTORY.reduce((sum, item) => sum + item.price * item.soldQty, 0);
  const totalSold = SOLD_HISTORY.reduce((sum, item) => sum + item.soldQty, 0);
  const totalListed = SOLD_HISTORY.length;

  const displayedHistory = showAllHistory ? SOLD_HISTORY : SOLD_HISTORY.slice(0, 4);
  ...
  return (
    <LinearGradient colors={['#DAE6D8', '#92AF8C']} style={styles.container}>
      ...
      {sidebarOpen && (
        <View style={styles.sidebarDrawer}>
          ...
          <View style={styles.navList}>
            <Pressable style={[styles.navItem, styles.navItemActive]} onPress={() => { setSidebarOpen(false); router.push('/home'); }}>
          ...
          <View style={styles.logoutWrapper}>
            <Pressable style={styles.logoutButton} onPress={() => { setSidebarOpen(false); router.push('/'); }}>
      ...
      <Modal visible={showTimePickerModal} transparent animationType="slide" onRequestClose={() => setShowTimePickerModal(false)}>
        <Pressable style={styles.modalOverlay} onPress={() => setShowTimePickerModal(false)}>
            ...
            <ScrollView style={styles.timeOptionsList}>
              {TIME_OPTIONS.map((time) => (
                <Pressable
                  ...
                  onPress={() => { setSelectedTime(time); setShowTimePickerModal(false); }}
                >
                  <Text style={[styles.timeOptionText, selectedTime === time && styles.timeOptionTextActive]}>{time}</Text>
                  {selectedTime === time && <Ionicons name="checkmark" size={20} color="#fff" />}
                </Pressable>
              ))}
            </ScrollView>
          </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        {activeTab === 'add' && (
          <View style={styles.formCard}>
            ...
            <Pressable style={styles.photoUploader} onPress={handlePickImage}>
              {photo ? (
                ...
                <View style={styles.photoPlaceholder}>
                  <MaterialCommunityIcons name="camera-plus-outline" size={32} color="#7A8A7A" />
                  <Text style={styles.photoPlaceholderText}>Tap to upload photo</Text>
                  <Text style={styles.photoPlaceholderHint}>Help buyers see what they're getting</Text>
                </View>
              )}
            </Pressable>
            ...
            <TextInput style={styles.input} placeholder="e.g., Special Fried Rice" placeholderTextColor="#A8A29E" value={foodName} onChangeText={setFoodName} />

            {/* Description */}
            <Text style={[styles.fieldLabel, { marginTop: 16 }]}>
              Description <Text style={styles.optional}>(optional)</Text>
            </Text>
            <TextInput style={[styles.input, styles.textarea]} placeholder="Ingredients, taste, when it was made..." placeholderTextColor="#A8A29E" value={description} onChangeText={setDescription} multiline textAlignVertical="top" />
            ...
            <View style={styles.inputRow}>
              <View style={styles.inputGroup}>
                ...
                <TextInput style={styles.input} placeholder="0" placeholderTextColor="#A8A29E" keyboardType="numeric" value={stockQty} onChangeText={setStockQty} />
              </View>
              <View style={styles.inputGroup}>
                <Text style={styles.fieldLabel}>Sale Price (Rp)</Text>
                <TextInput style={styles.input} placeholder="0" placeholderTextColor="#A8A29E" keyboardType="numeric" value={price} onChangeText={setPrice} />
              </View>
            </View>

            {/* Original Price */}
            <Text style={[styles.fieldLabel, { marginTop: 16 }]}>
              Original Price <Text style={styles.optional}>(optional, shown as strikethrough)</Text>
            </Text>
            <TextInput style={styles.input} placeholder="e.g., 30000" placeholderTextColor="#A8A29E" keyboardType="numeric" value={originalPrice} onChangeText={setOriginalPrice} />

            {/* Available Until */}
            <Text style={[styles.fieldLabel, { marginTop: 16 }]}>
              Available Until <Text style={styles.optional}>(pick date & time)</Text>
            </Text>
            <View style={styles.dateTimeRow}>
              ...
              <Pressable style={styles.timePickerButton} onPress={() => setShowTimePickerModal(true)}>
          ...
          <View style={styles.overviewContainer}>
            ...
            <View style={styles.statsRow}>
              ...
              <View style={[styles.statCard, styles.statCardHighlight]}>
                <Text style={styles.statValueHighlight}>Rp{formatPrice(totalRevenue)}</Text>
            ...
            <View style={styles.historyHeader}>
              ...
              <Pressable onPress={() => setShowAllHistory(v => !v)}>
                <Text style={styles.historyLink}>{showAllHistory ? 'Show less' : 'View all'}</Text>
              </Pressable>
            </View>
                    ...
                    <View style={styles.historyInfo}>
                      ...
                      <View style={styles.historyMeta}>
                        <View style={styles.metaItem}>
                          <MaterialCommunityIcons name="tag-outline" size={11} color="#7A8A7A" />
                          <Text style={styles.metaText}>{item.category}</Text>
                        </View>
                        <View style={styles.metaItem}>
                          <Ionicons name="calendar-outline" size={11} color="#7A8A7A" />
                          <Text style={styles.metaText}>{item.date}</Text>
                        </View>
                      </View>
                      <View style={styles.historyFooter}>
                        <Text style={styles.historyPrice}>Rp{formatPrice(item.price)}</Text>
                        <Text style={styles.historyQty}>{item.soldQty}/{item.totalQty} sold</Text>
                      </View>
                    </View>
                  </View>
                );
            ...
            {showAllHistory && (
              <Pressable style={styles.showLessBtn} onPress={() => setShowAllHistory(false)}>
...
const styles = StyleSheet.create({
  ...
  storeIconBox: { width: 42, height: 42, borderRadius: 12, backgroundColor: '#fff', alignItems: 'center', justifyContent: 'center', ...shadowStyle },
  greetingLabel: { fontSize: 11, color: '#5F6B5F' },
  greetingName: { fontSize: 18, fontWeight: '700', color: '#1F3A2E', marginTop: 1 },
  ...
  photoUploader: { width: '100%', height: 160, borderRadius: 12, backgroundColor: '#F9FAF9', borderWidth: 1.5, borderColor: '#E5E7E5', borderStyle: 'dashed', overflow: 'hidden', marginBottom: 16 },
  photoPreview: { width: '100%', height: '100%' },
  photoOverlay: { position: 'absolute', bottom: 0, left: 0, right: 0, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 8, backgroundColor: 'rgba(0,0,0,0.55)' },
  ...
  categoryChip: { paddingHorizontal: 14, paddingVertical: 7, borderRadius: 999, backgroundColor: '#F3F4F3' },
  ...
  datePickerButton: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#F9FAF9', borderRadius: 10, paddingHorizontal: 14, paddingVertical: 12, borderWidth: 1, borderColor: '#E5E7E5' },
  datePickerText: { fontSize: 14, color: '#1F3A2E', flex: 1 },
  timePickerButton: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#F9FAF9', borderRadius: 10, paddingHorizontal: 14, paddingVertical: 12, borderWidth: 1, borderColor: '#E5E7E5' },
  timePickerText: { fontSize: 14, color: '#1F3A2E', flex: 1 },

  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' },
  modalContent: { backgroundColor: '#fff', borderTopLeftRadius: 20, borderTopRightRadius: 20, maxHeight: '80%' },
  modalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 20, borderBottomWidth: 1, borderBottomColor: '#E5E7E5' },
  modalTitle: { fontSize: 18, fontWeight: '700', color: '#1F3A2E' },
  timeOptionsList: { paddingHorizontal: 20 },
  timeOption: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: '#F3F4F3' },
  timeOptionActive: { backgroundColor: '#324D3E', paddingHorizontal: 16, marginHorizontal: -16, borderRadius: 8 },
  timeOptionText: { fontSize: 16, color: '#1F3A2E' },
  timeOptionTextActive: { color: '#fff', fontWeight: '600' },

  launchButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: '#324D3E', borderRadius: 12, paddingVertical: 16, marginTop: 24, ...Platform.select({ ios: { shadowColor: '#324D3E', shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.3, shadowRadius: 10 }, android: { elevation: 6 }, default: { boxShadow: '0 6px 12px 0 rgba(50,77,62,0.3)' } }) },
  launchButtonText: { fontSize: 15, fontWeight: '700', color: '#fff' },

  overviewContainer: { gap: 14 },
  statsRow: { flexDirection: 'row', gap: 10 },
  statCard: { flex: 1, backgroundColor: '#fff', borderRadius: 14, padding: 12, ...shadowStyle },
  ...
  historyHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 8, marginBottom: 2 },
  historyTitle: { fontSize: 14, fontWeight: '700', color: '#1F3A2E' },
  historyLink: { fontSize: 12, fontWeight: '700', color: '#246DC1' },

  historyCard: { backgroundColor: '#fff', borderRadius: 14, padding: 12, flexDirection: 'row', gap: 12, ...shadowStyle },
  historyImage: { width: 64, height: 64, borderRadius: 10 },
  historyInfo: { flex: 1, justifyContent: 'space-between' },
  historyTopRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 8 },
  ...
  showLessBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 12, backgroundColor: '#fff', borderRadius: 12, marginTop: 4, ...shadowStyle },
  ...
  sidebarDrawer: { position: 'absolute', top: 0, left: 0, bottom: 0, width: 280, backgroundColor: '#fff', zIndex: 20, ...Platform.select({ ios: { shadowColor: '#000', shadowOffset: { width: 4, height: 0 }, shadowOpacity: 0.25, shadowRadius: 50 }, android: { elevation: 10 }, default: { boxShadow: '4px 0 50px 0 rgba(0,0,0,0.25)' } }) },
